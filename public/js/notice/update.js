...
this.op.url= op.url||"/notice/update.html#"+jsv+"@"+(location.hostname||"x");
if (op.l)
  this.op.url= op.url||"/notice/"+ll+"/update.html#"+jsv+"@"+(location.hostname||"x");
...
if (op.exp || (ll==="en" && !this.op.test && Math.round(Math.random()*200)<1)) { //test new script
     ...
     e.setAttribute("src", "/notice/updatex.js");
...
//console.log(this.op.browser.v,this.op.vs[this.op.browser.n]);
//console.log(!this.op.test,!this.op.browser,!this.op.browser.n,this.op.browser.n=="x",this.op.browser.donotnotify!==false,document.cookie.indexOf("browserupdateorg=pause")>-1,this.op.browser.v>this.op.vs[this.op.browser.n]);
...
if (languages.indexOf(ll)>0)
    this.op.url="/notice/update.html#"+jsv+"@"+(location.hostname||"x");
var tar="";
...
background:#FDF2AB no-repeat 13px center url(/img/notice/small/"+this.op.browser.n+".gif);\
