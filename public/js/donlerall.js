...
angular.module('donler.components.richComment', ['angularFileUpload','maggie.emoji'])
  ...
  .controller('RichCommentCtrl', ['$scope', '$http', '$window', '$element','$timeout', 'Comment', 'Report', 'FileUploader',
    function ($scope, $http, $window, $element, $timeout, Comment, Report, FileUploader) {
      $scope.pages = [];
      $scope.nowPage = 0;
      ...
      $scope.emoji = ["laugh", "smile", "happy", "snag", "snaky", "heart_eyes", "kiss", "blush", "howl", "angry",
      ...
      $scope.addEmotion = function(emotion) {
        // console.log(emotion);
        $scope.new_comment.text += '['+ dict[emotion] +']';
      };

      $window.onclick = function(event) {
        if(event.toElement.className.indexOf('emoji')===-1) {
          $scope.hideEmotions();
          $scope.$apply();
        }
      };
