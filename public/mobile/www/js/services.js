  ...
  var login = function($scope) {
    return function(username, password) {
      function initPushwoosh(callback) {
        //callback(null,null,'ERROR',[window.plugins,window.plugins.pushNotification]);
        ...
        document.addEventListener('push-notification', function(event) {
          ...
          pushNotification.registerDevice(function(status) {
                          ...
                          function(status) {
                                                ...
                                                callback(null,token,'ERROR',status);
                          });
          pushNotification.setApplicationIconBadgeNumber(0);
          pushNotification.getTags(function(tags) {
                       ...
                       function(error) {
                        console.warn('get tags error: ' + JSON.stringify(error));
                          //执行loginPost,将token POST到后台
                          callback(null,token,'ERROR',error);
                       });
      ...
      function loginPost(ids,token,status,error){
      ...
      if(ionic.Platform.device().platform == 'Android'){
        ...
        //console.log('PLATFORM-------',ionic.Platform.device().platform);
