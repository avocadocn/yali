...
exports.sendCompanyActiveMail = function (who, name, id, host) {
  ...
  var content = '<p>我们收到您在动梨的申请信息，请点击下面的链接来激活帐户：</p>' +
    '<a href="http://' + host + '/company/validate?key=' + encrypt.encrypt(id,config.SECRET) + '&id=' + id + '">http://' + host + '/company/validate?key=' + encrypt.encrypt(id,config.SECRET) + '&id=' + id + '</a>';

    fs.readFile(rootConfig.root+'/app/views/partials/mailTemplate.jade', 'utf8', function (err, data) {
        if (err) throw err;
        console.log(data);
        var fn = jade.compile(data);
        var html = fn({'title':'注册激活','who':who,'content':content});
        console.log(html);
        sendMail({
          from: from,
          to: to,
...
exports.sendStaffResetPwdMail = function(who, uid, host) {
  ...
  var content = '<p>我们收到您在动梨的密码重置申请信息，请点击下面的链接来重置密码（30分钟内有效）：</p>' +
    ...
    fs.readFile(rootConfig.root+'/app/views/partials/mailTemplate.jade', 'utf8', function (err, data) {
        ...
        console.log(data);
        var fn = jade.compile(data);
        var html = fn({'title':'重置密码','who':who,'content':content});
        console.log(html);
        sendMail({
          from: from,
          to: to,
